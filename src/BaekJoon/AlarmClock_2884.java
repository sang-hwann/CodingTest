...
//        예제 입력 2 -        0 30 /         예제 출력 2 -        23 45

import java.util.Scanner;

public class AlarmClock_2884 {
    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in); // 입력받기위한 Scanner 객체 선언
        int nhour = scanner.nextInt(); //입력 , 변수저장 시간
        int m = scanner.nextInt(); //입력 , 변수저장 분
        //입력시간이 45분보다 작으면
        if ((m - 45) < 0) {
            if (nhour == 0) {
                nhour = 23; // 0시간일 경우 24에서 -1인데 그냥 바로 23으로 수정
            } else {
                nhour -= 1; //1시간 줄이기
            }
            m = 60 + (m - 45); //그리고 60분에서 차이값만큼 빼기
        } else {
            m -= 45; //그외에는 그냥 45빼기
        }
        System.out.println(nhour + " " + m); //결과 출력
